...
for (var i in hi) {
  ...
  if (message.guild) {
    ...
    if (!score) {
      score = { id: `${message.guild.id}-${message.author.id}`, user: message.author.id, guild: message.guild.id, java: 0, python: 0, rust:0, discordjs:0, discordpy:0, c:0, cplus:0, csharp:0, javascript:0, html:0, php:0, sys:0, database:0, arduino:0, lua:0, seo:0, asm:0, warns:0}
    }
  ...
  if(command === "lb") {
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .addFields({name: "Exemple: \nLangage", value: `<:embed:801174015406374933> @Pseudo du référent • **Nombre** d'aides`, inline: false})
      .setColor(0x020000);

    for(const data of javascript) {
      embed.addFields({name: "<:js:791680343698309121> Javascript", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.javascript}** aides`, inline: false});
    }
    for(const data of python) {
      embed.addFields({name: "<:python:791680344272666624> Python", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.python}** aides`, inline: false});
    }
    for(const data of java) {
      embed.addFields({name: "<:java:792105873915904030> Java", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.java}** aides`, inline: false});
    }
    for(const data of rust) {
      embed.addFields({name: "<:rust:807948690035441685> Rust", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.rust}** aides`, inline: false});
    }
    for(const data of discordjs) {
      embed.addFields({name: "<:discordjs:807948702328160257> Discord.js", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.discordjs}** aides`, inline: false});
    }
    for(const data of discordpy) {
      embed.addFields({name: "<:discordpy:807948674063400960> Discord.py", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.discordpy}** aides`, inline: false});
    }
    for(const data of c) {
      embed.addFields({name: "<:c_:791659815906705438> C", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.c}** aides`, inline: false});
    }
    for(const data of cplus) {
      embed.addFields({name: "<:cplus:791659815990591538> C++", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.cplus}** aides`, inline: false});
    }
    for(const data of csharp) {
      embed.addFields({name: "<:csharp:791680343886921728> C#", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.csharp}** aides`, inline: false});
    }
    for(const data of html) {
      embed.addFields({name: "<:htmlcss:807948674184642590> HTML/CSS", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.html}** aides`, inline: false});
    }
    for(const data of php) {
      embed.addFields({name: "<:php:792105887661555712> PHP", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.php}** aides`, inline: false});
    }
    for(const data of sys) {
      embed.addFields({name: "<:sysadmin:807948673655898172> Sys-Admin", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.sys}** aides`, inline: false});
    }
    for(const data of database) {
      embed.addFields({name: "<:db:807948694430547998> Database", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.database}** aides`, inline: false});
    }
    for(const data of arduino) {
      embed.addFields({name: "<:Arduino:807948681382199316> Arduino/Raspberry", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.arduino}** aides`, inline: false});
    }
    for(const data of lua) {
      embed.addFields({name: "<:lua:808287061631041566> Lua", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.lua}** aides`, inline: false});
    }
    for(const data of seo) {
      embed.addFields({name: "<:seo:807948680879407106> SEO", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.seo}** aides`, inline: false});
    }
    for(const data of asm) {
      embed.addFields({name: "<:asm:807948700777054218> ASM", value: `<:embed:801174015406374933> ${client.users.cache.get(data.user)} • **${data.asm}** aides`, inline: false});
    }
  ...
  if(command === "rank" || command === "lvl" || command === "level") {
    ...
    const embed = new Discord.MessageEmbed()
      ...
      if(userscore.javascript != 0){
        embed.addFields({name: `<:js:791680343698309121> Javascript`, value: `<:embed:801174015406374933> **${userscore.javascript}** aides`, inline: false})
      }
      if(userscore.python != 0){
        embed.addFields({name: `<:python:791680344272666624> Python`, value: `<:embed:801174015406374933> **${userscore.python}** aides`, inline: false})
      }
      if(userscore.java != 0){
        embed.addFields({name: `<:java:792105873915904030> Java`, value: `<:embed:801174015406374933> **${userscore.java}** aides`, inline: false})
      }
      if(userscore.rust != 0){
        embed.addFields({name: `<:rust:807948690035441685> Rust`, value: `<:embed:801174015406374933> **${userscore.rust}** aides`, inline: false})
      }
      if(userscore.discordjs != 0){
        embed.addFields({name: `<:discordjs:807948702328160257> Discord.js`, value: `<:embed:801174015406374933> **${userscore.discordjs}** aides`, inline: false})
      }
      if(userscore.discordpy != 0){
        embed.addFields({name: `<:discordpy:807948674063400960> Discord.py`, value: `<:embed:801174015406374933> **${userscore.discordpy}** aides`, inline: false})
      }
      if(userscore.c != 0){
        embed.addFields({name: `<:c_:791659815906705438> C`, value: `<:embed:801174015406374933> **${userscore.c}** aides`, inline: false})
      }
      if(userscore.cplus != 0){
        embed.addFields({name: `<:cplus:791659815990591538> C++`, value: `<:embed:801174015406374933> **${userscore.cplus}** aides`, inline: false})
      }
      if(userscore.csharp != 0){
        embed.addFields({name: `<:csharp:791680343886921728> C#`, value: `<:embed:801174015406374933> **${userscore.csharp}** aides`, inline: false})
      }
      if(userscore.html != 0){
        embed.addFields({name: `<:htmlcss:807948674184642590> HTML/CSS`, value: `<:embed:801174015406374933> **${userscore.html}** aides`, inline: false})
      }
      if(userscore.php != 0){
        embed.addFields({name: `<:php:792105887661555712> PHP`, value: `<:embed:801174015406374933> **${userscore.php}** aides`, inline: false})
      }
      if(userscore.sys != 0){
        embed.addFields({name: `<:sysadmin:807948673655898172> Sys-Admin`, value: `<:embed:801174015406374933> **${userscore.sys}** aides`, inline: false})
      }
      if(userscore.database != 0){
        embed.addFields({name: `<:db:807948694430547998> Database`, value: `<:embed:801174015406374933> **${userscore.database}** aides`, inline: false})
      }
      if(userscore.arduino != 0){
        embed.addFields({name: `<:Arduino:807948681382199316> Arduino/Raspberry`, value: `<:embed:801174015406374933> **${userscore.arduino}** aides`, inline: false})
      }
      if(userscore.lua != 0){
        embed.addFields({name: `<:lua:808287061631041566> Lua`, value: `<:embed:801174015406374933> **${userscore.lua}** aides`, inline: false})
      }
      if(userscore.seo != 0){
        embed.addFields({name: `<:seo:807948680879407106> SEO`, value: `<:embed:801174015406374933> **${userscore.seo}** aides`, inline: false})
      }
      if(userscore.asm != 0){
        embed.addFields({name: `<:asm:807948700777054218> ASM`, value: `<:embed:801174015406374933> **${userscore.asm}** aides`, inline: false})
      }
...
if(command === "resetwarn"){
  ...
  if(command === "vote"){
    ...
    const membre = message.mentions.members.first()
    const channel = client.channels.cache.find(channel => channel.id === '812304842647273534')
    const score = client.getScore.get(membre.id, message.guild.id);
...
if (emoji.id == '807948690035441685') {
  score.rust++;
  client.setScore.run(score);
  channel.send(`[LOG] : ${user.tag} a réagit avec l'émoji ${emoji} sur le message de <@${reaction.message.author.tag}> dans le salon <#${reaction.message.channel.id}>\nLien : https://discord.com/channels/789670704911613992/${reaction.message.channel.id}/${reaction.message.id}`)
}
